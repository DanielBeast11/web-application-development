...
@swagger_auto_schema(
    ...
    responses={
        status.HTTP_200_OK: openapi.Response(
            schema=CarsSerializer(many=True),
            description=""
        )
    }
)
...
def create_car(request):
    serializer = CarAddSerializer(data=request.data, partial=False)
...
@swagger_auto_schema(
    ...
    responses={
        status.HTTP_200_OK: openapi.Response(
            schema=DepreciationsSerializer(many=True),
            description=""
        )
    }
)
...
@swagger_auto_schema(
    ...
    responses={
        status.HTTP_200_OK: openapi.Schema(
            ...
            properties={
                "cars_count": openapi.Schema(type=openapi.TYPE_NUMBER),
                "draft_depreciation": openapi.Schema(type=openapi.TYPE_NUMBER)
            },
            required=["cars_count", "draft_depreciation"]
        )
    }
)
@api_view(["GET"])
...
def update_summ(request, depreciation_id):
    if not Depreciation.objects.filter(pk=depreciation_id).exists():
        return Response(status=status.HTTP_404_NOT_FOUND)

    depreciation = Depreciation.objects.get(pk=depreciation_id)
    depreciation.summ = request.data.get("value")
    depreciation.save()
...
@api_view(["PUT"])
@permission_classes([IsModerator])
def update_status_admin(request, depreciation_id, mileage, initial_price):
    if not Depreciation.objects.filter(pk=depreciation_id).exists():
        return Response(status=status.HTTP_404_NOT_FOUND)
    ...
    if request_status == 3:
        calculate_summ(depreciation_id, mileage, initial_price)
...
def calculate_summ(depreciation_id, mileage, initial_price):
    data = {
    ...
    requests.post("http://remote_service:8080/calculate_summ/", json=data, timeout=3)
...
@swagger_auto_schema(
    ...
    responses={
        status.HTTP_200_OK: openapi.Response(
            schema=CarItemSerializer(many=True),
            description=""
        )
    }
)
...
@swagger_auto_schema(
    ...
    request_body=openapi.Schema(
        type=openapi.TYPE_OBJECT,
        properties={
            'mileage': openapi.Schema(type=openapi.TYPE_NUMBER),
        },
        required=["mileage"],
    ),
    responses={
        status.HTTP_200_OK: openapi.Response(
            schema=CarDepreciationSerializer,
            description=""
        )
    }
)
